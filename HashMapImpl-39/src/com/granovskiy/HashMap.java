...
public class HashMap implements Map {
    ...
    private Object processFullNodeEmptyList(Node node, Bucket bucketSearched) {
        ...
        checkIfCollisionIs(node, bucketSearched);  //  !!!
    ...
    private boolean checkIfCollisionIs(Node node, Bucket bucketSearched) {
        ...
        for (int i = 0; i < bucketSearched.getNodeList().size(); i++) {
            if (node.getKey().hashCode() == bucketSearched.getNodeList().get(i).hashCode()) {
                booleanResult = true;
            }
        }
...
/*    Object objectToBeReturned = null;
        for (int i = 0; i < bucketSearched.getNodeList().size(); i++) {
        if (node.getKey().hashCode() == bucketSearched.getNodeList().get(i).hashCode()) {
            objectToBeReturned = bucketSearched.getNodeList().get(i).hashCode();
            bucketSearched.getNodeList().add(i, node);
        }
    }
