...
public class HashMap implements Map<Car, Driver> {
    ...
    public Driver get(Object key) {
        ...
        int bucketNumber = carFromMainMethod.hashCode() %  mapSize;
        System.out.println("bucket: " + bucketList.get(bucketNumber));
        Bucket bucket = bucketList.get(bucketNumber);
        System.out.println();
        Driver result = null;
        if (bucket.getNode() != null && carFromMainMethod.equals(bucket.getNode().getCar())) {
            result =  bucket.getNode().getDriver();
        }

        if (bucket.getNode() == null && carFromMainMethod.equals(bucket.getNodeList().get(bucketNumber).getCar())) {
            result = (Driver) bucket.getNodeList().get(bucketNumber).getDriver();
        }
        return result;
    }
